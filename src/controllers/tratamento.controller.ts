  ...
  try {
    ...
    if (!pacienteId || !Diagnostico || !Data_inicio) {
      return res.status(400).json({
        error:
          "Campos obrigatórios ausentes: pacienteId, Diagnostico, Data_inicio",
      });
    }
      ...
      select: {
        ...
        tratamento_medicamento: {
          select: {
            ...
            Quantidade_comprimidos: true,
            Instrucoes: true,
            Data_Inicio_Tratamento: true,
            Data_Fim_Tratamento: true,
            Observacoes_Acompanhamento: true,
            Efeitos_Colaterais: true,
            Adesao: true,
            farmaceutico: {
              select: {
                ID: true,
                Nome_Farmaceutico: true,
                Sobrenome_Farmaceutico: true,
              },
            },
            tratamento_medicamento_has_medicamentos: {
              select: {
                medicamentos: {
                  select: {
                    ID: true,
      ...
      select: {
        ...
        tratamento_medicamento: {
          select: {
            ...
            Efeitos_Colaterais: true,
            Adesao: true,
            Data_Ultimo_Acompanhamento: true,
            Data_Proximo_Acompanhamento: true,
            farmaceutico: {
              select: {
                ID: true,
                Nome_Farmaceutico: true,
...
export const deletarTratamento = async (req: Request, res: Response) => {
  try {
    const ID = Number(req.params.id);
    if (!ID)
      return res.status(400).json({ error: "ID do tratamento é obrigatório" });
